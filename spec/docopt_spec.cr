...
describe Util::ParseUtil do
  describe ".tokenize_option_line" do
    it "returns empty string on an empty string" do
      Util::ParseUtil.tokenize_option_line("").should eq("")
    end

    it "returns the flag specified on the line" do
      Util::ParseUtil.tokenize_option_line("-a").should eq("-a")
      Util::ParseUtil.tokenize_option_line("-v").should eq("-v")
    end

    it "treats text after 2 spaces as a comment" do
      Util::ParseUtil.tokenize_option_line("--verbose  This is a comment").should eq("--verbose")
    end
  ...
  describe ".parse_option_lines" do
    it "returns an empty hash on an empty array" do
      Util::ParseUtil.parse_option_lines([] of String).should eq([] of String)
    end

    it "gets rid of leading 'options:' strings" do
      Util::ParseUtil.parse_option_lines(["options:  --foo"]).should eq(["--foo"])
    end

    it "works when comments include ':'" do
      Util::ParseUtil.parse_option_lines(
  ...
  describe ".get_args_from_usage_lines" do
    it "gets a single argument" do
      Util::ParseUtil.parse_usage_lines(["usage: prog <args>"]).should eq(["<args>"])
    end
  end
end
